...
const PairVsModal = ({ isModalOpen, handleCloseModal, matchInfo }: Props) => {
  ...
  const isUserParticipating = () => {
    ...
    const team1Players = [
      matchInfo?.pair1Id?.player1,
      matchInfo?.pair1Id?.player2,
    ].filter(Boolean)
    const team2Players = [
      matchInfo?.pair2Id?.player1,
      matchInfo?.pair2Id?.player2,
    ].filter(Boolean)
  ...
  const teamTwoPlayers: {
    ...
    email?: string
    profileImage?: string
  }[] = [] // Since we're only showing the current user, team 2 will be empty
